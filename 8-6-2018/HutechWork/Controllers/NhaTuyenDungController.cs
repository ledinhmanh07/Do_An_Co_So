...
namespace HutechWork.Controllers
{
    public class NhaTuyenDungController : Controller
    {
        ...
        public ActionResult Sua(int id, FormCollection fr)
        {
            var a = db.CHITIETTUYENDUNGs.SingleOrDefault(n => n.MAPDT == id);
            a.TIEUDE = fr["tieude"];
            ...
            b.THOIHANDANGTIN = DateTime.Parse(String.Format("{0:MM/dd/yyyy}", fr["thoihan"]));
            db.SubmitChanges();
            return RedirectToAction("ChiTietDT", "NhaTuyenDung",new {id = id});
        }
    }
}